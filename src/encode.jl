...
function write_record_complex(encoder::DBNEncoder, record)
    ...
    elseif isa(record, CMBP1Msg)
        ...
        write(io, UInt8(record.side))
        write(io, record.flags)
        write(io, record._reserved1)
        write(io, record.ts_recv)
        write(io, record.ts_in_delta)
        for byte in record._reserved2
            write(io, byte)
        end
        ...
        write(io, record.levels.bid_sz)
        write(io, record.levels.ask_sz)
        write(io, record.levels.bid_pb)
        write(io, record.levels._reserved1[1])
        write(io, record.levels._reserved1[2])
        write(io, record.levels.ask_pb)
        write(io, record.levels._reserved2[1])
        write(io, record.levels._reserved2[2])
